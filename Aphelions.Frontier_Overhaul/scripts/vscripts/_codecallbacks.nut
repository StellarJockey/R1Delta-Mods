...
function CodeCallback_OnEntityKilled( entity, damageInfo )
{
	...
	// --- Stealth bonus: +5 XP only if the actual damage source had a silencer AND the NPC could NOT see the attacker ---
	...
	if ( IsValid( attacker ) && attacker.IsPlayer() && !attacker.IsTitan() && !entity.IsPlayer() && !entity.IsTitan() )
	{
		...
		// Case A: direct weapon entity caused the damage
		if ( IsValid( weaponFromDI ) )
		{
			local wpnMods = weaponFromDI.GetMods()
			foreach ( m in wpnMods )
			{
				if ( m == "silencer" )
				{
					gotSilencer = true
		...
		// Case B: inflictor is a projectile spawned by a weapon (check mods on projectile)
		else if ( IsValid( inf ) && inf.IsProjectile() )
		{
			local infMods = inf.GetMods()
			foreach ( m in infMods )
			{
				if ( m == "silencer" )
				{
					gotSilencer = true
		...
		// Only award if the actual damage-causing entity had a silencer
		if ( gotSilencer )
		{
			if ( !entity.CanSee( attacker ) )
			{
				AddXP( 5, attacker )

				if ( "StealthBonus" in level.scoreEventsByName )
				{
					local ev = level.scoreEventsByName["StealthBonus"]
					local scoreEventInt = ev.GetInt()
					Remote.CallFunction_NonReplay( attacker, "ServerCallback_PointSplash", scoreEventInt, null, 5 )
				}
