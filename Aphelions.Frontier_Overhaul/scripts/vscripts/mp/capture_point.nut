...
function InitializeHardpointsForCapturePoint()
{
	...
	hardpointArray = SortHardpointsByGroup( hardpointArray )

	// do gamemode independed init
	InitializeHardpoints( hardpointArray )
...
function CapturePoint_Update( hardpoint )
{
	...
	if ( players.len() > 0 )
	{
		...
		{
			// PLAYER IS MILITIA: 
			// Enemy (IMC) grunts get 3.5x buff
			imcPower = ( imcPlayers + imcAI ) * CAPTURE_POINT_PLAYER_CAP_POWER * 3.5
		...
		{
			// PLAYER IS IMC: 
			// Enemy (Militia) grunts get 3.5x buff
			milPower = ( milPlayers + milAI ) * CAPTURE_POINT_PLAYER_CAP_POWER * 3.5
