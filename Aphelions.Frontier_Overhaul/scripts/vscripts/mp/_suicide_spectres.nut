...
function SpectreSuicideOnDamaged( spectre, damageInfo )
{
	...
	if ( damageInfo.GetCustomDamageType() & damageTypes.Electric )
	{
	...
	{
		// Instant explosion when stepped on by a Titan or melee'd
		case eDamageSourceId.titan_step:
		case eDamageSourceId.titan_melee: 	
			thread SpectreExplode( spectre, results )
			return
			
		// Electrical things and instant-kills neutralize immediately
