...
function EvacShipMain( health = EVAC_DROPSHIP_HEALTH, shield = EVAC_DROPSHIP_SHIELD_HEALTH )
{
	...
	// --- INSTANT TAKEOFF ---
	local departureTime = Time() + GetTimeTillDepartureAnim()
...
function EvacModeShouldEnd()
{
	...
	if ( evacPlayers.len() == 0 )
		{
			// Abort the evac for everything UNLESS it's Campaign
	...
	if ( GetNumberOfEvacPlayersAlive() == 0 )
	{
		// Campaign is immune to being cut off by player deaths here
		if ( playlist == "campaign_carousel" )
			return false

		// For everything else EXCEPT Attrition and Hardpoint, end the evac immediately
		if ( playlist != "at" && playlist != "cp" && playlist != "ctf" )
			return true

		// For Attrition, only end if the player is truly eliminated (no more respawns)
		local players = GetPlayerArrayOfTeam( level.evacTeam )
		local anyCanRespawn = false
