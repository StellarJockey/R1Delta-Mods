...
{
    // don't operate on entities that are invalid this frame
    if ( !IsValid_ThisFrame( guy ) )
    {
        printt( "Warning: __PlayAnim called with invalid guy: " + guy + " anim: " + animation_name )
        return
    }

    local animation = GetAnim( guy, animation_name )

    // entity may have been destroyed during GetAnim
    if ( !IsValid_ThisFrame( guy ) )
    {
        printt( "Warning: __PlayAnim: guy invalidated after GetAnim(): " + guy + " anim: " + animation_name )
        return
    }
