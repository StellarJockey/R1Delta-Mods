...
function MapButton_Focused( button )
{
	...
	if (GetModeNameForEnum(level.ui.privatematch_mode) == "campaign_carousel") {
		...
		local customDescriptions = {
			...
			mp_harmony_mines = "Energy-rich ores are extracted at this industrial mining facility run by Kodai Industries, located on the planet Harmony. Graphite and tungsten carbide are instrumental for Titan production.",
			mp_corporate = "Applied Robotics labs on the Frontier, such as this one, developed the first automated infantry 'Spectre' units. Hammond Robotics is an IMC Premier Technology Company, though many secrets are still hidden under NDAs.",
			...
			mp_npe = "Simulation Training Pods are used for Pilot certification exams, though many have been cracked and distributed by underground criminal networks. Remember, piracy is a crime."
		}
