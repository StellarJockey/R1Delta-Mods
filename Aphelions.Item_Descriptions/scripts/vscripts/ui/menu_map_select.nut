...
function MapButton_Focused( button )
{
	...
	nextMapImage.SetColor( 170, 170, 170 )
	if (GetModeNameForEnum(level.ui.privatematch_mode) == "campaign_carousel") {
		...
		local customDescriptions = {
			...
			mp_outpost_207 = "Orbital defense cannons stationed at high altitude defend against the incursion of hostile capital ships. This outpost is responsible defending an IMC shipyard in the Freeport system.",
			mp_airbase = "IMC Airbase Sierra is defended against local wildlife by the latest generation of repulsor tower technology. It is the single largest airfield in the Frontier, set on the fourth moon of the planet Demeter.",
			...
			mp_colony = "IMC and Militia forces clash in the close-quarters of an uncharted rural colony, built from the scrapped parts of the ghost ship, IMS Odyssey. After a massacre was committed by IMC Spectres, it has since been abandoned.",
			mp_angel_city = "When the IMC instituted martial law in Angel City, massive walls were built to divide the city into many smaller districts. It has recently entered the tenth year of its temporary 'two-week' lockdown.",
			...
			mp_rise = "Militia special forces set up a Long-Range Desert Patrol outpost in an abandoned IMC reservoir, not far from Training Ground Whitehead. Gridiron is a hostile planet, baked by solar radiation.",
			mp_boneyard = "Extensive IMC research on wildlife repulsor technology was conducted at this facility, many years ago. Its existence has since been purged from all written records.",
			mp_training_ground = "IMC Pilot training programs conducted here have a 98 percent fatality rate. Only the strong survive - assuming their numbers are to be trusted. The IMC are known for their propaganda.",
			mp_haven = "A luxury retreat for the wealthy built on the edge of a massive crater lake. This world is home to many offshore accounts, whose owners have a vested interest in seeing the Frontier's war continue.",
			mp_swampland = "Drainage operations have revealed ancient ruins of unknown origin. Vice Admiral Spyglass dispatches a search team to investigate, ordering that this information be kept secret...",
			...
			mp_harmony_mines = "Energy-rich ores are extracted at this small industrial mining facility, located on the planet Harmony. Graphite and tungsten are instrumental for Titan production.",
			mp_corporate = "Applied Robotics labs on the Frontier, such as this one, developed the IMC's Spectre automated infantry unit. Hammond Robotics holds many company secrets - all buried under NDAs.",
			mp_lagoon = "An IMC carrier makes an emergency landing on a small fishing village in the Freeport system, though it is highly unlikely they're here to ask the locals for directions.",
			mp_backwater = "High in the mountains, ex-IMC pilot Barker and his fellow colonists have made a comfortable living by producing moonshine in this hidden bootlegging colony. It brings back memories of simpler times, before the Frontier War.",
		}
