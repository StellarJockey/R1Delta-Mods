...
const CP_REINFORCE_INTERVAL = 0.5
...
function SpawnCapturePointSquad( team, numFreeSlots )
{
	...
	else
	{
		...
		foreach( hardpoint in shuffledPoints )
		{
			...
			// AGGRESSIVE CODE
			local owned = hardpoint.GetTeam() == team
			// NPCs will now consider attacking any point not owned by their team, 
			// even if no friendly player is there to lead them.
			if ( !owned ) 
			{
				// Point is neutral or enemy-held; allow spawning to capture it
			}
			else
			{
				// Point is already owned; check if it needs reinforcements (hasPlayer or low AI count)
