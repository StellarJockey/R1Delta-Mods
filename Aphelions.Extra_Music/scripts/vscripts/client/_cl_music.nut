...
{
	// Diagnostic print - should appear every time any music starts
	printt( ">>> CALLBACK RECEIVED | ID: " + musicPieceID + " | SEEK: " + shouldSeek )

	// If it's the intro, trigger our custom logic thread
	if ( musicPieceID == eMusicPieceID.LEVEL_INTRO )
	{
		thread HandleClientSideIntro()
	}

	// DO NOT RETURN. We must let the original logic run (even if it's muted)
	// so that the game's internal timers for action music keep running.
	if ( shouldSeek == false )
	{
	...
	if ( shouldSeek == true && IntroMusicHasFinishedPlaying( musicPieceID, timeMusicStarted, seekTime ) )
		delaythread ( 1.0 ) PlayActionMusic()
	else
...
function HandleClientSideIntro()
{
	// Wait briefly for the client to catch up
	wait 0.1 
	local player = GetLocalClientPlayer()
	
	if ( !IsValid( player ) )
	{
		printt( ">>> Custom Intro Error: Player not valid" )
		return
	}
	...
	if ( team == TEAM_IMC )
	{
		standardAlias = "Music_IMC_Intro_Standard"
		longPool = [ "Music_IMC_Intro_Long_1", "Music_IMC_Intro_Long_2" ]
	}
	else if ( team == TEAM_MILITIA )
	{
		standardAlias = "Music_Militia_Intro_Standard"
		longPool = [ "Music_Militia_Intro_Long_1", "Music_Militia_Intro_Long_2" ]
	}
	else 
	{
		return 
	...
	local choice = RandomInt( longPool.len() + 1 )
	local aliasToPlay = ( choice == 0 ) ? standardAlias : longPool[ choice - 1 ]
	
	printt( ">>> Playing Custom Intro: " + aliasToPlay )
	
	if ( choice == 0 )
		EmitSoundOnEntity( player, aliasToPlay )
	else
		thread PlayAndFadeCustomTrack( player, aliasToPlay )
}

// Your fade logic transfers over exactly as you wrote it
