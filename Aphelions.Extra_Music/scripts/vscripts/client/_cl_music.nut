...
function PlayMusic_Internal( musicPieceID, seek = 0 )
{
	...
	if ( musicPieceID == eMusicPieceID.LEVEL_INTRO )
		thread PlayIntroMusicWhenDone( lengthOfMusic )
	else
...
function PlayNextActionMusicWhenDone( lengthOfMusic )
{
	...
	level.ent.EndSignal( "MusicPlayed" ) // <-- THIS kills any old background music threads!
	level.ent.EndSignal( "MusicStopped" )

	local playingAlias = level.currentMusicPlaying

	// Wait for the actual length of the sound file
	wait lengthOfMusic

	if ( level.currentMusicPlaying == playingAlias )
	{
		level.currentMusicPlaying = null
		level.currentClassMusicType = null
	}
...
function PlayIntroMusicWhenDone( lengthOfMusic )
{
	level.ent.Signal( "MusicPlayed" )
	level.ent.EndSignal( "MusicPlayed" ) // <-- THIS kills any old background music threads!
	level.ent.EndSignal( "MusicStopped" )
	...
	if ( IsValid( player ) && introAlias != null )
	{
		FadeOutSoundOnEntity( player, introAlias, fadeTime )
	}
	...
	level.lastTimeActionMusicPlayed = 0 // Bypass the 30s debounce
	thread PlayActionMusic()
}
